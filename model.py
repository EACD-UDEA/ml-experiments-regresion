...
def get_estimator_mapping():
 ...
 #       "ridge-regressor": Ridge,
        "average-cost-per-status-regressor": AverageCostPerStatus,
        "twovar-regressor": TwoVarRegressor,
        "categorical-encoder": CategoricalEncoder,
        "standard-scaler": StandardScaler,
        "discretizer": Discretizer,
        "averager": AverageCostPerStatusExtractor,
        "twomean-extractor": TwoMeanExtractor
    }
...
class AverageCostPerStatus(BaseEstimator, RegressorMixin):
    def fit(self, X, y):
        """Computes the mode of the cost per smoking status on training data."""
        df = pd.DataFrame({"smoker": X["smoker"], "cost": y})
        self.means_ = df.groupby("smoker").mean().to_dict()["cost"]
...
class TwoVarRegressor(BaseEstimator):
    def fit(self, X, y):
        """Computes the mean of y per smoking status and sex on training data."""
        df = pd.DataFrame({"smoker": X["smoker"], "sex": X["sex"], "y": y})
        self.means_ = df.groupby(["smoker", "sex"]).mean().to_dict()["y"]
        self.global_mean_ = y.mean()
        return self
    ...
    def predict(self, X):
        ...
        y_pred = X[["smoker", "sex"]].apply(tuple, axis=1).apply(get_mean)
        return y_pred


class AverageCostPerStatusExtractor(BaseEstimator, TransformerMixin):
    def fit(self, X, y):
        df = pd.DataFrame({"smoker": X["smoker"], "cost": y})
        self.means_ = df.groupby("smoker").mean().to_dict()["cost"]
        self.global_mean_ = y.mean()
    ...
    def transform(self, X):
        ...
        X["AverageCostBySmokingStatus"] = X["smoker"].apply(get_average)
        return X


class TwoMeanExtractor(BaseEstimator, TransformerMixin):
    def fit(self, X, y):
    ...
    def transform(self, X):
        ...
        X["TwoVarRegressor"] = X[["smoker", "sex"]].apply(tuple, axis=1).apply(get_mean)
        return X
