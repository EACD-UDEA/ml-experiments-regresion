...
def _get_test_dataset(data_config, splits):
    filepath = data_config["filepath"]
    reader = partial(pd.read_csv, filepath_or_buffer=filepath)
    return data.get_test_dataset(reader=reader, splits=splits)
...
def _save_versioned_estimator(
    ...
    try:
        ...
        typer.echo(f"Couldn't serialize model due to error {e}")
...
def predict(
    ...
    saved_model = os.path.join(output_dir, model_version, "model.joblib")
    estimator = joblib.load(saved_model)
    dataset = _get_test_dataset(_load_config(config_file, "datatest"), splits=splits)
    predictions = list()
    ...
    results = {"id": "SebasMonsalve","email": "sebastian.monsalves@udea.edu.co", "task_type": "Regression" ,"predictions": predictions}
